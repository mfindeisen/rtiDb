...
/*
  MODIFICATIONS FOR SPA (Vue.js) INTEGRATION:
  - Replaced `sglRegisterCanvas` with `sglRegisterLoadedCanvas` in `createRtiViewer`.
    This fixes an issue where SpiderGL waits for `window.onload`, which never fires dynamically in an SPA.
  - Changed relative paths for shaders (e.g. "spidergl/box.v.glsl") and icons (e.g. "css/icons/zoomin.png")
    to absolute paths from the root ("/viewer/spidergl/..." and "/viewer/css/icons/...").
    This prevents the SPA router from incorrectly prepending the current page route to the paths.
*/
...
function createRtiViewer(idDiv, imageUrl, width, height)
{
	...
	var containerEl = document.getElementById(idDiv);
	if (!containerEl) {
		return;
	}
	var canvasContainer = $(containerEl);
	...
	divHelp.innerHTML = '<div id = "guideTable"><div id = "guideCell"> <div id = "guideList"><h3>WebRTIViewer<br/></h3><ul><li>Pan: LeftMouseButton + MouseMove.</li><li>Zoom in: MouseWhell or press the button <span><img src = "/viewer/css/icons/zoomin.png" alt=""/></span></li><li>Zoom out: MouseWheel or press the button <span><img src = "/viewer/css/icons/zoomout.png" alt=""/></span></li><li>To change the light direction: activate the light with the button <span><img src = "/viewer/css/icons/light.png" alt=""/></span> and press LeftMouseButton + MouseMove or Ctrl + LeftMouseButton + MouseMove.</li><li>Fullscreen: press the button <span><img src = "/viewer/css/icons/full.png" alt=""/></span> to active the fullscreen mode and the button <span><img src = "/viewer/css/icons/full_on.png" alt=""/></span> to exit</li><li>To reset the viewpoint: press R.</li></ul><a href="http://vcg.isti.cnr.it/rti/webviewer.php" target = "_black">Visual Computing Lab ISTI CNR Pisa</a></div></div></div>'
	...
	else
	{
		...
		sglRegisterLoadedCanvas(canvasName, multiResRTI, 10.0);
	}
}
...
function MultiResRenderer(gl, cacheSizeInBytes, onLoad)
 {
	...
	// box rendering
	/******************************************************/
	var bvs = sglLoadFile("/viewer/spidergl/box.v.glsl");
	var bfs = sglLoadFile("/viewer/spidergl/box.f.glsl");
...
MultiResRenderer.prototype = {
	...
	loadImage : function (url)
	{
		...
		if (!doc) {
			console.error("Renderer.loadImage failed to parse info.xml, responseText was:", xhttp.responseText);
			return;
		}
		...
		{
			case 1:
				var vs = sglLoadFile("/viewer/spidergl/multi.v.glsl");
				var fs = sglLoadFile("/viewer/spidergl/hsh.f.glsl");
			...
			case 2:
				var vs = sglLoadFile("/viewer/spidergl/multi.v.glsl");
				var fs = sglLoadFile("/viewer/spidergl/lrgbptm.f.glsl");
				var prg = new SglProgram(this.gl, [vs], [fs]);
				log(prg.log);
				this._program  = prg;
				break;
			case 3:
				var vs = sglLoadFile("/viewer/spidergl/multi.v.glsl");
				var fs = sglLoadFile("/viewer/spidergl/rgbptm.f.glsl");
				var prg = new SglProgram(this.gl, [vs], [fs]);
				log(prg.log);
				this._program  = prg;
				break;
			case 4:
				var vs = sglLoadFile("/viewer/spidergl/multi.v.glsl");
				var fs = sglLoadFile("/viewer/spidergl/image.f.glsl");
				var prg = new SglProgram(this.gl, [vs], [fs]);
				log(prg.log);
				this._program  = prg;
				break;
