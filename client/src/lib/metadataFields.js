...
const DATE_METADATA_KEYS = new Set(['digitalRegistrationDate', 'imagingDate']);
...
export function formatCatalogDateTime(value) {
  if (value == null || value === '') return '';
  const s = String(value).trim();
...
export function emptyMetadata() {
  return Object.fromEntries(ALL_METADATA_KEYS.map((k) => [k, '']));
}
...
export function getFilledMetadata(data) {
 ...
 */
export function parseGpsPosition(text) {
...
export function gpsMapsUrl(text) {
  const coords = parseGpsPosition(text);
  return coords ? googleMapsUrl(coords.lat, coords.lng) : null;
}

/** Extract first #RGB or #RRGGBB from text; returns normalized #RRGGBB or null. */
