...
import type { ImageSearchMatch } from './imageSearch.js';

export function hashImageFile(filePath: string): Promise<string> {
...
export function getIndexedCatalogCount(): number {
  const row = db
...
export interface CachedImageSearch {
  ...
  catalogCountAtSearch: number;
  currentCatalogCount: number;
}

export function getCachedImageSearch(contentHash: string, limit: number): CachedImageSearch | null {
  const row = db
    .select()
  ...
  return {
    results: row.results as ImageSearchMatch[],
    total: row.total,
...
export function saveCachedImageSearch(contentHash: string, limit: number, results: ImageSearchMatch[]): void {
  const now = new Date().toISOString();
  const catalogCount = getIndexedCatalogCount();
