...
export function ensureRecordsSchema(sqlite) {
  const cols = new Set(sqlite.pragma('table_info(records)').map((c) => c.name));
  ...
  const indexes = new Set(
    sqlite.pragma('index_list(records)').map((idx) => idx.name)
  );
...
export function ensureAnnotationSchema(sqlite) {
  const cols = new Set(sqlite.pragma('table_info(record_annotations)').map((c) => c.name));
  if (!cols.has('source')) {
