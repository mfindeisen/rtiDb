...
export function ensureCommentSchema(sqlite: Database.Database) {
  const tableExists = sqlite
    .prepare("SELECT 1 AS ok FROM sqlite_master WHERE type = 'table' AND name = 'record_comments'")
    .get();
  ...
  `);
  console.log('Schema repair: created record_comments table');
}
